...
const { mostLikelyCommand } = require('./commands/mostlikely');
const { startMotMelange, checkMotMelange } = require('./commands/motmelange');
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    try {
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.locate'): {
                const locateArgs = userMessage.split(' ').slice(1);
                await locateCommand(sock, chatId, locateArgs, message);
                break;
            }
            case userMessage.startsWith('.aura'): {
                const auraArgs = userMessage.split(' ').slice(1);
                await auraCommand(sock, chatId, senderId, auraArgs, message);
                break;
            }
            case userMessage.startsWith('.plussusceptible'): {
                await mostLikelyCommand(sock, chatId, message, isGroup);
                break;
            }
            case userMessage.startsWith('.motmelange'): {
                await startMotMelange(sock, chatId, message);
                break;
            }
            case userMessage.startsWith('.trouve'): {
                const trouveArg = userMessage.split(' ').slice(1).join(' ');
                await checkMotMelange(sock, chatId, senderId, trouveArg, message);
                break;
            }
